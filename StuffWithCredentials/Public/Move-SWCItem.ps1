<#
.SYNOPSIS
    Move files with credentials.
.DESCRIPTION
    A wrapper for Move-Item, with credentials support.
.EXAMPLE
    PS C:\> Move-SWCItem -Path .\testfile.txt -Destination C:\
    This will move the testfile.txt from the current director to the C drive.
.EXAMPLE
    PS C:\> Move-SWCItem -Path .\testfile.txt -Destination \\server1\c$ -Credential (Get-Credential)
    This will move the testfile.txt from the current director to C drive on the server1 where you need another set of credentials to get write access.

#>
function Move-SWCItem {
    [CmdletBinding()]
    Param
    (
        #Help
    ...
    Process {
        ...
        try {
            ...
            Write-Verbose "Moving the file/folder $($Path -split '\\' | Select-Object -last 1)"
            Move-Item -Path $Path -Destination "$($PSDriveName):" -Force:$Force -Recurse:$Recurse -ErrorAction Stop
        }
